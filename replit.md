...
- **Python script**: `artifacts/api-server/src/lib/analyze_binary.py`
  ...
  - **Binary Diff**: `artifacts/api-server/src/lib/binary_diff.py` — compares two binaries: classes, methods, symbols, strings, libraries, sections, functions, Mach-O headers, codesign. Smart string categorization (Features, APIs, Services, Errors, Localization, Security, UI Text, Debug) with garbage filtering. Auto-generates human-readable insights. Includes 7 analysis panels: Security Risk Assessment (scored 0-100), Privacy Impact (trackers, frameworks), Network Footprint (URLs/domains/IPs), Entitlements Diff (permissions, URL schemes, background modes, ATS), Code Signature (Team ID, provisioning profile, re-sign detection), Function-level Diff (added/removed/modified with size changes), Mach-O Header Diff (flags, load commands, SDK version, PIE/ASLR, rpaths). Export: JSON + styled HTML report (dark theme, XSS-safe).
