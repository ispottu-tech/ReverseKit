...
- Pages: `src/pages/` — each page is a self-contained component
  ...
  - `binary-diff.tsx` — Binary Diff: compare two iOS binaries, shows changes in classes, methods, symbols, strings, libraries, sections. Export buttons (JSON + HTML report with XSS-safe escaping). Entitlements Diff panel (violet theme) for permission changes.
...
- **Python script**: `artifacts/api-server/src/lib/analyze_binary.py`
  ...
  - **Binary Diff**: `artifacts/api-server/src/lib/binary_diff.py` — compares two binaries: classes, methods, symbols, strings, libraries, sections. Smart string categorization (Features, APIs, Services, Errors, Localization, Security, UI Text, Debug) with garbage filtering. Auto-generates human-readable insights (new frameworks, code growth, service changes). Includes 4 analysis panels: Security Risk Assessment (SSL pinning, anti-debug, jailbreak detection, encryption changes scored 0-100), Privacy Impact (tracking frameworks, SDKs like Firebase/Facebook/Adjust/AppsFlyer, device fingerprinting, data collection patterns), Network Footprint (URL/domain/IP comparison between versions), Entitlements Diff (IPA/deb entitlement extraction + Info.plist metadata — compares permissions, URL schemes, background modes, ATS settings). Export: JSON data dump + styled HTML report (dark theme, XSS-safe).
