...
def enhanced_class_dump(path):
    """Full class-dump reconstruction — complete ObjC interface with typed properties, method signatures, categories"""
    try:
        import lief
        binary = lief.parse(path)
        if binary is None:
            return {"error": "Cannot parse binary", "dump": "", "classes": [], "protocols": [], "categories": []}
        ...
        if not classes:
            return {"dump": "// No Objective-C class metadata found.", "classes": [], "protocols": [], "categories": [], "stats": {}}

        all_symbols = []
        try:
            for sym in main.symbols:
                all_symbols.append(str(sym.name))
        except Exception:
            pass
        ...
        def decode_type(enc):
            ...
            enc = str(enc).strip()
            if enc.startswith('@"'):
                cls_name = enc[2:].rstrip('"')
                return f"{cls_name} *" if cls_name else "id"
            if enc in type_map:
                return type_map[enc]
            if enc.startswith('{'):
                struct_name = enc.split('=')[0].lstrip('{')
                return f"struct {struct_name}" if struct_name else "struct"
            return "id"

        def parse_prop_attrs(attrs_str):
            if not attrs_str:
                return [], "id"
            parts = str(attrs_str).split(',')
            objc_attrs = []
            prop_type = "id"
            for p in parts:
                p = p.strip()
                if p.startswith('T'):
                    ...
                    objc_attrs.append('readonly')
                elif p == 'C':
                    objc_attrs.append('copy')
                elif p == '&':
                    objc_attrs.append('strong')
                elif p == 'W':
                    objc_attrs.append('weak')
                elif p == 'N':
                    objc_attrs.append('nonatomic')
                elif p == 'D':
                    objc_attrs.append('@dynamic')
                elif p.startswith('G'):
                    objc_attrs.append(f'getter={p[1:]}')
                elif p.startswith('S'):
                    objc_attrs.append(f'setter={p[1:]}')
            if not objc_attrs:
                objc_attrs = ['nonatomic', 'strong']
            return objc_attrs, prop_type

        def format_method_sig(method, prefix):
            m_name = str(method.name)
            parts = m_name.split(':')
            if len(parts) <= 1:
                return f"{prefix} (id){m_name};"
            sig_parts = []
            for i, part in enumerate(parts):
                if not part:
                    continue
                if i == 0:
                    sig_parts.append(f"{prefix} (id){part}:(id)arg{i}")
                else:
        ...
        lines = []
        lines.append("//")
        lines.append("//  Generated by ReverseKit — class-dump reconstruction")
        lines.append(f"//  Binary: {os.path.basename(path)}")
        ...
        for cls in classes:
            try:
                if hasattr(cls, 'super_class') and cls.super_class:
                    sc = str(cls.super_class.name) if hasattr(cls.super_class, 'name') else str(cls.super_class)
                    if sc and sc not in ('NSObject', 'UIViewController', 'UIView', 'UITableViewCell'):
                        forward_decls.add(sc)
            except Exception:
                pass

        if forward_decls:
            for fd in sorted(forward_decls):
                lines.append(f"@class {fd};")
            lines.append("")
        ...
        for cls in classes:
            cls_name = str(cls.name)
            detail = {"name": cls_name, "methods": 0, "properties": 0, "ivars": 0}

            protocols = []
            try:
                protocols = [str(p) for p in cls.protocols]
                all_protos.update(protocols)
            except Exception:
                pass
                    ...
                    for prop in cls.properties:
                        ...
                        try:
                            if hasattr(prop, 'attribute'):
                                attrs_str = str(prop.attribute)
                        except Exception:
                            pass
                        attrs, prop_type = parse_prop_attrs(attrs_str)
                        attr_str = ", ".join(attrs) if attrs else "nonatomic, strong"
                        lines.append(f"@property ({attr_str}) {prop_type} {prop_name};")
                        properties.append(prop_name)
            except Exception:
                pass
            detail["properties"] = len(properties)
            ...
            for m in class_methods:
                lines.append(format_method_sig(m, "+"))
            for m in instance_methods:
                lines.append(format_method_sig(m, "-"))

            detail["methods"] = len(class_methods) + len(instance_methods)
        ...
        for sym in all_symbols:
            cat_match = re.match(r'_OBJC_\$_CATEGORY_(\w+)_\$_(\w+)', sym)
            if cat_match:
                categories.append({"class": cat_match.group(1), "category": cat_match.group(2)})
                lines.append(f"@interface {cat_match.group(1)} ({cat_match.group(2)})")
                lines.append("@end")
                lines.append("")

        stats = {
            "total_classes": len(classes),
            "total_methods": sum(d["methods"] for d in class_details),
            "total_properties": sum(d["properties"] for d in class_details),
            "total_ivars": sum(d["ivars"] for d in class_details),
            "total_protocols": len(all_protos),
        ...
        return {
            ...
            "protocols": sorted(list(all_protos)),
            "categories": categories,
            "stats": stats,
        }
    except Exception as e:
        return {"error": str(e), "dump": "", "classes": [], "protocols": [], "categories": [], "stats": {}}


def detect_encrypted_strings(path):
    """Detect and attempt to decrypt obfuscated/encrypted strings in binary"""
    import re
    import base64
    ...
    try:
        ...
        ascii_strings = out.stdout.strip().split("\n") if out.stdout else []
    except Exception:
        pass
    ...
    for s in ascii_strings:
        s = s.strip()
        if b64_pattern.match(s) and len(s) >= 16:
            try:
                decoded = base64.b64decode(s)
                text = decoded.decode("utf-8", errors="strict")
                if text.isprintable() and len(text) >= 4:
                    results["base64_decoded"].append({
                        "encoded": s[:80],
                        "decoded": text[:200],
                        "length": len(s),
                    })
    ...
    for s in ascii_strings:
        s = s.strip()
        if hex_pattern.match(s) and len(s) >= 16 and len(s) % 2 == 0:
            try:
                decoded = bytes.fromhex(s)
                text = decoded.decode("utf-8", errors="strict")
                if text.isprintable() and len(text) >= 4:
                    results["hex_encoded"].append({
                        "encoded": s[:80],
                        "decoded": text[:200],
                    })
            except Exception:
    ...
    url_words = {'http', 'https', 'www', '.com', '.io', '.net', 'api', 'token', 'key', 'secret', 'password', 'auth'}
    for s in ascii_strings:
        s = s.strip()
        if len(s) < 8:
            continue
        r = rot13(s)
        for w in url_words:
            if w in r.lower() and w not in s.lower():
                results["rot13_decoded"].append({
                    "encoded": s[:80],
                    "decoded": r[:200],
                    "matched_keyword": w,
                })
    ...
    for key_byte in range(1, 256):
        xor_hits = []
        for s in ascii_strings[:500]:
            s = s.strip()
            if len(s) < 10 or len(s) > 200:
                continue
            try:
                xored = bytes([b ^ key_byte for b in s.encode('ascii')])
                text = xored.decode('ascii', errors='strict')
                if text.isprintable():
                    for w in url_words:
                        if w in text.lower() and w not in s.lower():
                            xor_hits.append({
                                "encoded": s[:60],
                                "decoded": text[:200],
                ...
                pass
        if xor_hits:
            results["xor_candidates"].extend(xor_hits[:5])
        if len(results["xor_candidates"]) >= 20:
            break

    entropy_threshold = 5.5
    chunk_size = 64
    import math
    for i in range(0, min(len(raw), 500000), chunk_size):
        ...
        entropy = -sum((c / chunk_size) * math.log2(c / chunk_size) for c in freq.values() if c > 0)
        if entropy > entropy_threshold:
            has_printable = sum(1 for b in chunk if 32 <= b <= 126) / chunk_size
            if 0.3 < has_printable < 0.9:
                preview = ''.join(chr(b) if 32 <= b <= 126 else '.' for b in chunk[:32])
                results["suspicious_blobs"].append({
                    "offset": f"0x{i:X}",
                    "entropy": round(entropy, 2),
                    "preview": preview,
                    "printable_ratio": round(has_printable, 2),
                })
                if len(results["suspicious_blobs"]) >= 15:
                    break

    results["total_found"] = (
        len(results["base64_decoded"]) +
        len(results["xor_candidates"]) +
        len(results["hex_encoded"]) +
        len(results["rot13_decoded"]) +
        len(results["suspicious_blobs"])
    )
...
def _analyze_binary(path):
    ...
    # 20. Enhanced Class-dump
    result["class_dump"] = enhanced_class_dump(path)

    # 21. String Decryption Analysis
    result["string_decrypt"] = detect_encrypted_strings(path)
