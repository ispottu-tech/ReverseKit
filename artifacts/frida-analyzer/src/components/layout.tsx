...
import { Home, ScanSearch, Binary, BookMarked, Smartphone, TerminalSquare, Crosshair } from "lucide-react";
...
export function Layout({ children }: { children: React.ReactNode }) {
  ...
  const nav = [
    { href: "/", label: "Home", icon: Home, section: null },
    { href: "/binary", label: "Binary Inspector", icon: ScanSearch, section: "Static Analysis" },
    { href: "/hex", label: "Hex Viewer", icon: Binary, section: null },
    { href: "/scripts", label: "Script Arsenal", icon: BookMarked, section: "Tools" },
    { href: "/device", label: "Device Manager", icon: Smartphone, section: "Dynamic Analysis" },
  ];
  ...
  return (
    <div className="flex h-screen w-full bg-background text-foreground font-sans selection:bg-primary/30 dark">
      <aside className="w-56 border-r border-border bg-sidebar flex-shrink-0 flex flex-col z-10">
        <div className="h-14 flex items-center px-5 border-b border-border/60 gap-3">
          ...
          <div>
            ...
            <div className="text-[10px] text-muted-foreground/60 font-mono leading-none mt-0.5">v1.0</div>
          </div>
        </div>
              ...
              <div key={item.href}>
                {showSection && (
                  <div className="px-3 mt-4 mb-2">
                    <span className="text-[10px] uppercase tracking-[0.12em] text-muted-foreground/50 font-semibold">{item.section}</span>
                  </div>
                )}
        ...
        <div className="p-3 border-t border-border/50">
          <Link href="/device" className={cn(
            "flex items-center gap-2.5 px-3 py-2.5 rounded-md border text-xs font-mono cursor-pointer transition-all",
            isConnected
